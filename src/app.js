...
import { fibonacciSphere, hashText } from './util.js';
...
const formStateHint = document.getElementById('formStateHint');
...
const MODEL_PARTICLE_TARGET = AUTO_SETTINGS.modelParticleCount;
...
function currentSettings() {
    return {
        particleCount: readControlNumber(particleCount, AUTO_SETTINGS.particleCount),
        speed: readControlNumber(simSpeed, AUTO_SETTINGS.speed),
        intensity: readControlNumber(glowIntensity, AUTO_SETTINGS.intensity),
        particleSize: readControlNumber(particleSize, AUTO_SETTINGS.particleSize),
        surfaceFlow: readControlNumber(surfaceFlow, AUTO_SETTINGS.surfaceFlow),
        mousePush: readControlNumber(mousePush, AUTO_SETTINGS.mousePush),
        brightness: readControlNumber(hologramBrightness, AUTO_SETTINGS.brightness),
        modelRotate: modelRotate?.checked ?? AUTO_SETTINGS.modelRotate,
        showRings: showRings?.checked ?? AUTO_SETTINGS.showRings,
        showGrid: showGrid?.checked ?? AUTO_SETTINGS.showGrid,
        palette: currentPalette()
    };
}

function readControlNumber(element, fallback) {
    return Number(element?.value) || fallback;
}

function updateControlLabels() {
    const settings = currentSettings();
    if (particleCountValue) particleCountValue.textContent = settings.particleCount.toLocaleString();
    if (particleReadout) particleReadout.textContent = `${settings.particleCount.toLocaleString()} particles`;
    if (simSpeedValue) simSpeedValue.textContent = settings.speed.toFixed(2);
    if (glowIntensityValue) glowIntensityValue.textContent = settings.intensity.toFixed(2);
    if (particleSizeValue) particleSizeValue.textContent = settings.particleSize.toFixed(3);
    if (surfaceFlowValue) surfaceFlowValue.textContent = settings.surfaceFlow.toFixed(2);
    if (mousePushValue) mousePushValue.textContent = settings.mousePush.toFixed(2);
    if (hologramBrightnessValue) hologramBrightnessValue.textContent = settings.brightness.toFixed(2);
}
...
function applySettings(spec) {
    ...
    const finalParticleCount = autoDensityForSpec(spec, requestedParticleCount);
...
function autoDensityForSpec(spec, requestedParticleCount) {
    if (spec.kind === 'black_hole_3d') return 90000;
    if (spec.sceneGraph?.objects?.some((object) => object.type === 'model_points')) return MODEL_PARTICLE_TARGET;
    if (spec.sceneGraph?.objects?.some((object) => object.type === 'particle_shape')) return Math.max(90000, requestedParticleCount);
    if (spec.title === 'Chaos Core') return AUTO_SETTINGS.particleCount;
    return Math.max(AUTO_SETTINGS.particleCount, requestedParticleCount);
}

function withSharedStageEffects(spec) {
    if (spec.kind !== 'scene_graph' || !spec.sceneGraph) return spec;
    const settings = currentSettings();
    const palette = spec.palette || settings.palette;
    const effects = spec.sceneGraph.effects || [];
    const hasHologramStage = effects.some((effect) => effect.type === 'hologram_stage');
    const hasVoiceStage = effects.some((effect) => effect.type === 'voice_stage');
    const sharedEffects = [
        !hasHologramStage && {
            id: 'hologram_stage',
            type: 'hologram_stage',
...
function createChaosCoreScene(metadata = {}) {
    const settings = currentSettings();
    const palette = settings.palette;
    const seed = hashText(`chaos-core:${settings.particleCount}:${settings.speed}:${appState.hologramTheme}`);
    const sphere = createOriginSphereMorph(settings.particleCount);

    return withSharedStageEffects(applySettings({
        renderer: '3d',
        kind: 'scene_graph',
        title: 'Breathing Sphere',
        palette,
        particleCount: settings.particleCount,
        labels: ['idle sphere', 'breathing surface', 'voice-reactive form'],
        seed,
        controls: { trail: true, glow: true, autoRotate: false },
        sceneGraph: {
            version: 1,
            duration: 18,
            world: { environment: 'dark_studio', timeScale: 1, physicsMode: 'visual' },
            objects: [
                {
                    id: 'chaos_core',
                    type: 'model_points',
                    ...
                    morphDuration: metadata.morphFrom ? 1.8 : 0,
                    position: [0, 1.2, 0],
                    particleCount: settings.particleCount,
                    sampled: true,
                    material: {
            ...
            objects: [
                {
                    ...
                    morphFromVertices: metadata.morphFrom?.vertices,
                    morphFromNormals: metadata.morphFrom?.normals,
                    morphViaVertices: metadata.morphVia?.vertices,
                    morphViaNormals: metadata.morphVia?.normals,
                    morphViaAt: metadata.morphVia ? 0.46 : 0,
                    morphDuration: metadata.morphFrom ? (metadata.morphVia ? 2.45 : 1.75) : 0,
                    particleCount: modelParticleCount,
...
async function setScene(scene, subtitle = '') {
    ...
    if (particleReadout) particleReadout.textContent = `${scene.particleCount.toLocaleString()} particles`;
    if (sceneSpec) sceneSpec.textContent = JSON.stringify(sceneForDisplay(scene), null, 2);
    stage.dataset.renderer = '3d';
    if (specStatus) specStatus.textContent = 'VALID';
...
function setFormState(mode, hint) {
    ...
    const hints = {
        idle: 'Breathing origin sphere online',
        listening: 'Absorbing your voice input',
        thinking: 'Mapping meaning into form',
        speaking: 'Voice output driving particle waves',
        forming: 'Rebuilding the particle silhouette'
    };
    stage.dataset.voiceMode = normalized;
    if (formStateLabel) formStateLabel.textContent = labels[normalized] || labels.idle;
    if (formStateHint) formStateHint.textContent = hint || hints[normalized] || hints.idle;
    if (voiceCard) {
        voiceCard.classList.toggle('is-listening', normalized === 'listening');
        voiceCard.classList.toggle('is-thinking', normalized === 'thinking');
        voiceCard.classList.toggle('is-speaking', normalized === 'speaking');
    }
}

function handleVoiceState(state = {}) {
    appState.activeRenderer.setVoiceState(state);
    if (state.mode) setFormState(state.mode);
}
...
function projectActiveFormation() {
    ...
    if (appState.activeFormation === 'chaos') {
        setScene(createChaosCoreScene(), 'Idle particle consciousness');
        return;
    }
...
async function handleModelUpload(file) {
    ...
    if (!isSupportedModelFile(file.name)) {
        setSystemStatus('FORMAT');
        return;
    }

    try {
        setSystemStatus('IMPORTING');
        const model = await loadModelVertices(file, Math.max(currentSettings().particleCount, MODEL_PARTICLE_TARGET));
        const morphFrom = captureCurrentMorphSource();
        await loadModelData(file.name, model.vertices, model.format, {
            ...model,
            morphFrom,
            morphVia: createOriginSphereMorph(Math.max(currentSettings().particleCount, MODEL_PARTICLE_TARGET))
        });
    } catch (error) {
        console.warn('Model import failed.', error);
        setSystemStatus('IMPORT FAIL');
    }
}
...
async function loadBuiltInModel(modelId, { skipOrigin = false } = {}) {
    const preset = BUILT_IN_MODELS[modelId];
    if (!preset) return;
    const targetCount = Math.max(currentSettings().particleCount, MODEL_PARTICLE_TARGET);
    const cacheKey = `${modelId}:${targetCount}`;
    const morphFrom = skipOrigin ? null : captureCurrentMorphSource(modelId);

    try {
        setSystemStatus('LOADING MODEL');
        ...
        await loadModelData(preset.name, model.vertices, model.format || preset.format, {
            ...
            builtIn: modelId,
            morphFrom: morphFrom || createOriginSphereMorph(targetCount),
            morphVia: morphFrom ? createOriginSphereMorph(targetCount) : null
        });
...
function captureCurrentMorphSource(nextBuiltInId = null) {
    if (appState.activeFormation === 'model' && appState.uploadedModel?.vertices?.length) {
        return captureCurrentModelForMorph(nextBuiltInId);
    }
    return createOriginSphereMorph(Math.max(currentSettings().particleCount, MODEL_PARTICLE_TARGET));
}
...
function createOriginSphereMorph(count = MODEL_PARTICLE_TARGET) {
    const points = fibonacciSphere(count, 19);
    return {
        name: 'Origin Sphere',
        vertices: points,
        normals: points.map(([x, y, z]) => {
...
function clearActiveModelButtons() {
    document.querySelectorAll('[data-model]').forEach((button) => button.classList.remove('is-active'));
}

function setChaosCore(subtitle = 'Idle particle consciousness', metadata = {}) {
    const morphFrom = metadata.morphFrom || captureCurrentModelForMorph();
    appState.activeFormation = 'chaos';
    appState.uploadedModel = null;
    clearActiveModelButtons();
    setFormState('idle');
    return setScene(createChaosCoreScene({ morphFrom }), subtitle === 'Idle particle consciousness' ? 'Idle breathing origin sphere' : subtitle);
}

async function setVoiceScene(transcript) {
    const text = String(transcript || '').trim();
    if (!text) return;
    appState.activeFormation = 'voice_scene';
    appState.uploadedModel = null;
    clearActiveModelButtons();
    setFormState('forming', `Forming: ${text.slice(0, 34)}`);
    setScene(withSharedStageEffects(applySettings(parseScenePrompt(text))), `Interpreted from voice: ${text.slice(0, 42)}`);
}
...
function handleVoiceCommand(transcript) {
    ...
    if (/混沌|默认|待机|回来|回到|意识|orb|chaos|idle/.test(text)) {
        setChaosCore('Back to idle chaotic particle consciousness');
        handledShape = true;
    } else if (/bb[-\s]?8|bb8|球形机器人/.test(text)) {
        loadBuiltInModel('bb8');
        handledShape = true;
    } else if (/bd[-\s]?1|bd1|机器人/.test(text)) {
        loadBuiltInModel('bd1');
        handledShape = true;
    } else if (/spirit|core|核心|能量/.test(text)) {
        loadBuiltInModel('spirit');
        handledShape = true;
    } else if (/汽车|轿车|跑车|车辆|car|vehicle|sports car/.test(text)) {
        setVoiceScene(transcript);
        handledShape = true;
    }

    const theme = (() => {
        if (/紫|violet|purple/.test(text)) return 'violet';
        if (/白|亮|light|white/.test(text)) return 'light';
        if (/黄|金|橙|amber|orange/.test(text)) return 'amber';
        if (/青|蓝|cyan|blue/.test(text)) return 'cyan';
        return null;
    })();
    if (theme) {
        appState.hologramTheme = theme;
        document.querySelectorAll('[data-theme]').forEach((item) => item.classList.toggle('is-active', item.dataset.theme === theme));
        scheduleProjectActiveFormation();
    }

    if (/多一点|更多|密一点|increase|more/.test(text)) {
        if (particleCount) {
            particleCount.value = Math.min(MAX_PARTICLE_COUNT, Number(particleCount.value) + 20000);
            updateControlLabels();
            scheduleProjectActiveFormation();
        } else {
            setFormState('thinking', 'Density is automatically optimized');
        }
    }
    if (/少一点|降低|reduce|less/.test(text)) {
        if (particleCount) {
            particleCount.value = Math.max(MODEL_PARTICLE_TARGET, Number(particleCount.value) - 20000);
            updateControlLabels();
            scheduleProjectActiveFormation();
        } else {
            setFormState('thinking', 'Density is automatically optimized');
    ...
    const likelySceneDescription = /展示|显示|变成|形成|看|场景|形状|宇宙|太阳|月球|地球|城市|海洋|山|森林|黑洞|磁场|文字|车|show|display|become|form|scene|shape|city|ocean|mountain|forest|black hole|magnetic|text/.test(text);
    if (!handledShape && likelySceneDescription) {
        setVoiceScene(transcript);
    }
}
...
async function loadModelData(name, vertices, format = 'MODEL', metadata = {}) {
    if (vertices.length < 3) {
        setSystemStatus('NO VERTICES');
        return;
    }

    if (particleCount && Number(particleCount.value) < MODEL_PARTICLE_TARGET) {
        particleCount.value = MODEL_PARTICLE_TARGET;
        updateControlLabels();
    }

    if (!metadata.morphFrom) {
        metadata.morphFrom = createOriginSphereMorph(Math.max(currentSettings().particleCount, MODEL_PARTICLE_TARGET));
    }
...
function bindEvents() {
    ...
    [particleCount, simSpeed, glowIntensity, particleSize, surfaceFlow, mousePush, hologramBrightness].filter(Boolean).forEach((input) => {
    ...
    [modelRotate, showRings, showGrid].filter(Boolean).forEach((input) => {
